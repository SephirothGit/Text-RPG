...
func (c *BaseCharacter) ChooseLocation() {
	...
	for {
		...
		if err != nil {
			fmt.Printf("input error when choose a place to go: %v", err)
			return
		}
...
func (c *BaseCharacter) GoToForest() {
	...
	for {
		...
		if err != nil {
			fmt.Printf("Input error after meeting HobGoblin: %v", err)
		}
...
func (c *BaseCharacter) GoToTown() {
	...
	for {
		...
		if err != nil {
			fmt.Printf("Input error when choose a place to visit in town: %v", err)
			continue
		}
...
func (c *BaseCharacter) GoToTavern() {
	...
	for {
		...
		if err != nil {
			fmt.Printf("Input error when choose a tavern action: %v", err)
			continue
		}
...
func (c *BaseCharacter) GoToTavernB() {
	...
	for {
		...
		if err != nil {
			fmt.Printf("input err when choose a Brick's tavern action: %v", err)
			continue
		}
...
func (c *BaseCharacter) GoToDungeon() {
	...
	for {
		...
		if err != nil {
			fmt.Printf("Input error when choose a floor in dungeon: %v", err)
		}
...
func (c *BaseCharacter) GoToInn() {
	...
	for {
		...
		if err != nil {
			fmt.Printf("Input error sleep action: %v", err)
			continue
		}
		...
		for {
			...
			if err != nil {
				fmt.Printf("Input error inn action: %v", err)
				continue
			}
